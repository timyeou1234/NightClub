...
open class TagListView: UIView {
    ...
    private func rearrangeViews() {
        ...
        for (index, tagView) in tagViews.enumerated() {
            tagView.frame.size = tagView.getButtonSize()
    ...
    private func createNewTagViewAttribute(_ title: String) -> TagView {
        let tagView = TagView(titleAtr: title)
    ...
    @discardableResult
    open func addTagsWithAttribute(_ titles: [String]) -> [TagView] {
        var tagViews: [TagView] = []
        for title in titles {
            tagViews.append(createNewTagViewAttribute(title))
        }
        return addTagViews(tagViews)
    }
